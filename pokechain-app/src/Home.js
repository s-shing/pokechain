...
function displayPoke(pokemon){
  var ipfs = pokedict[pokemon]["image"]

  if (pokedict[pokemon]["stageOne"]){
  return (
    <article> 
      <h2>{pokemon}</h2>
      <img src={ipfs}/>
      <button onClick={() => mint(pokemon)}>Mint me!</button>
    </article>
  )
}
}
...
function mint(pokemon){
  pokecontract.registerPokemon(pokedict[pokemon]["id"])
}
async function trade(pokemon){
  if (await pokecontract.requestPokemon(pokedict[pokemon]["evoBase"]) != null){
    let cost = parseInt(await pokecontract.getCost(pokedict[pokemon]["evoBase"]))
    if (await yodacontract.approve(await pokecontract.ownerOf(pokedict[pokemon]["evoBase"]),cost*100)==null){
       await pokecontract.rejectPokemon(pokedict[pokemon]["id"],await signer.getAddress());
